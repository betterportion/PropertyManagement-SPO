...
Defined in `shared/schema.ts` using Drizzle, with Zod insert schemas generated by `drizzle-zod`. This file is the single source of truth for both server and client types. Thirty-one tables:
...
| `user_permissions` | One row per user, seventeen boolean flags (including the two finance flags, `canCompleteWalkthroughs` which is the resident-tier walkthrough grant, and `canManagePropertySetup` which gates the per-property setup checklist) plus `allowedRegions` (text array) | `userId` unique, cascades on user delete |
