...
| `walkthrough_items` | One line of a room's checklist — the sink, the smoke detector. **Condition and notes live here**, not on a photo, so an item nobody photographed can still be assessed. The three dismiss columns (`dismissedAt`, `dismissReason`, `dismissedByUserId`) are written only by the dismiss routes, like an asset snooze | `roomId` → `walkthrough_rooms`, cascades |
