...
| `actionItems.ts` | What the dashboard says needs attention, from schedules coming due, unpaid rent and deposits still held, open work on a house (one aggregated item per house, never one per request), plus the manual `tasks`. `buildActionItems` is pure — records plus `now` — so it tests without a database or a clock. |
| `regionSummary.ts` | The per-region rollup a national admin reads. Also pure. "Health" is operational load only; unpaid rent is reported beside it, never inside it. Open work is reported as two counts, `openRepairs` (type `request`) and `openJobs` (projects and capital projects), derived from the type column and nothing else. |
...
Defined in `shared/schema.ts` using Drizzle, with Zod insert schemas generated by `drizzle-zod`. This file is the single source of truth for both server and client types. Thirty-four tables:
...
| `maintenance_request_comments` | The thread on a request: body, `isInternal` (default **true**), the author kept three ways (id set-null, email, name) so a comment outlives the account, the relay pair — `relaySource` is what renders, `relayContactId` the optional link — and one optional file: `attachmentUrl` is the `/uploads/<key>` the attachment route returned and `attachmentName` the name the file had, which is the link text. Posted once, never edited; deletable, and deleting leaves the file in storage | `requestId` → `maintenance_requests` cascades; `authorUserId` → `users` set-null; `relayContactId` → `maintenance_contacts` set-null; indexed on `requestId`; `attachmentUrl` is looked up by `findUploadReferences` |
| `maintenance_request_bids` | One contractor's offer to do a project: `contactId` when the firm has a record or `vendorName` typed when it does not (one of the two is required), `amount`, `bidDate`, `notes`, and the quote as `documentUrl` (the `/uploads/<key>` the bid-document route returned) plus `documentName`. `accepted` is written by the accept route only, which un-accepts every other bid on the request in the same transaction. Deleting a bid leaves the file in storage | `requestId` → `maintenance_requests` cascades; `contactId` → `maintenance_contacts` set-null; indexed on `requestId`; `documentUrl` is looked up by `findUploadReferences` |
...
| `walkthroughs` | A dated inspection event for one house: date, type (move_in/move_out/additional/legacy — `additional` was `annual` until migration `0032`, and move-in is the default because every house gets one), status (draft/submitted/reviewed), who performed it. This is what makes a year-over-year comparison possible — rooms used to hang straight off a property | `propertyId` → `properties` cascades; `region`/`buildingAddress` denormalised for region scoping |
| `walkthrough_items` | One line of a room's checklist — the sink, the smoke detector. **Condition and notes live here**, not on a photo, so an item nobody photographed can still be assessed. The three dismiss columns (`dismissedAt`, `dismissReason`, `dismissedByUserId`) are written only by the dismiss routes, like an asset snooze. `standingNote` is copied forward like the room's, and a resident cannot write it | `roomId` → `walkthrough_rooms`, cascades |
| `walkthrough_rooms` | The rooms of one walkthrough, ordered by `displayOrder`. `requiredQuestions` is legacy — the backfill turned each entry into a `walkthrough_items` row and kept the array. `standingNote` is staff instruction copied forward to the next walkthrough's copy of the room | `walkthroughId` → `walkthroughs` cascades; `propertyId` → `properties` (loose, no FK) |
...
| `resource_links` | What SPO publishes to household leaders and stewards: title, URL, description, category. A null `region` means national; a region name limits it to that region's houses. `slotKey` (nullable, unique) binds a link to one of the three named places on the hub — see `shared/resourceHubSlots.ts`; a slotted link must be national. **Links, never documents** | referenced by nothing |
...
**A staff request's region is its house's region.** The create and edit routes look the house up by `buildingAddress` (`getPropertyByAddress`) and take `region` from it, whatever the body says; an address that is not a house is a 400. A request tagged with one region but filed against a house in another is invisible to that house's RA while the household can still read it — the region and the house are two copies of one fact, so the server keeps them one. An older request whose address no longer matches a house keeps the region it has. The forms mirror it: picking a house fills in the region and locks it, on the request and contact dialogs alike.
...
**One file per comment, and it inherits the comment's visibility.** A quote PDF or a photo of what the contractor found goes up through `POST /api/maintenance-requests/:id/attachments` — the request's own route rather than `/api/upload-doc`, because that one refuses residents outright and a household may attach a photo to the shared comment it may post. The permission is the whole post rule (`canPostComment` with shared visibility), run **before** the multipart parser like every upload route, with the document limits, the same magic-byte check and the same upload layer, so `document.uploaded` fires without the route adding an event. The route returns `{ url, name }`; the client holds the pair until Post and sends it as `attachmentUrl` + `attachmentName`. The create route accepts the URL only in the `/uploads/<key>` shape (`UPLOAD_URL_PATTERN` in `shared/schema.ts`) **and only when the `uploads` row says this caller stored it** — a file inherits the visibility of every record that points at it, so a body naming a vendor's W-9 would otherwise hand that W-9 to the household. Reading it back goes through `findUploadReferences` as a `maintenanceRequestComment` reference, authorized by `canReadComment`: a file on an internal comment is never served to a resident, even one who can open the request. A second file is a second comment. **Deleting a comment removes the row and leaves the file in storage** (known issue 1); the confirmation says so.

**A household posts shared, and only shared.** A resident account posts on the requests it may read — its own house's, or its own submission — and `canPostComment` is the read rule plus "shared only" with nothing more, so the 120-day closed window and the repairs-only type rule reach posting with nothing implemented twice: a leader cannot post on another house, on a request closed more than 120 days ago, or on a project. The create route **forces `isInternal: false` for a resident whatever the body says** — the household composer has no visibility control, so a body marked internal from that tier is a client mistake, not a grant, and it is stored shared rather than refused, the way the resident create route forces the type. Delete stays author-or-admin after the read rule, so a leader takes down their own comment and nobody else's. This is a resident write path, which is the shape of both historic authorization gaps here, and `routeAccess.test.ts` pairs every refusal with the write never reaching storage. The author — id, email and name — comes from the session and never from the body. Every posted comment is emailed to the people who can see it; that rule is under "Outbound email" below, and a household's comment flows through it unchanged: the housemate and the covering staff hear about it, the author does not.
...
**Open work on the property page** is the four groups — Repairs, Projects, Capital projects, Wishlist, in that fixed order — of everything not yet closed on one house. `groupOpenWork` in `client/src/lib/maintenanceFilters.ts` is the partition: the wishlist is pulled out **first** (anything at wishlist priority, whatever its type), and the rest fall into their type group, so every open item lands in exactly one group and the four counts add up to the open total. It is computed on the client over the request list the property page already fetches — no route and no server rollup — and `client/src/components/PropertyOpenWork.tsx` renders it above the full request table, keeping an empty group on screen with a line saying so. Staff page only; a resident's repairs stay where they already were.

**The property page's history table carries the same type and range filters as the Maintenance page**, both defaulting to everything (all types, all closed requests), because a house's history is where a completed capital project belongs beside its closed repairs; `matchesType` and `closedWithinRange` from `maintenanceFilters.ts` are the rules, so the two screens cannot disagree about what a range or a type means. The filters narrow the table only — the open-work groups above it are never filtered, because the range is about history — and a table emptied by a filter says "Nothing matches these filters" rather than claiming nothing was ever reported. Client-side view filters like the Maintenance page's: the list route reads no query string and the resident type rule stays the separate server-side condition it is.

The same grouping appears at two more scopes. The **Maintenance page** has an "Open work" tab (`?view=open`, in the URL like the other filters) rendering the same component over whatever the filters above it select — one house once a house is picked, which is the primary view: an RA works one house at a time. The **dashboard** reads open work through `actionItems.ts` as one aggregated item per house — "Open work — Cleveland House · 2 repairs, 1 project" — keyed on the house's **address** (what the building filter reads, and a request outlives its property record), with no due date so it sits below every dated item, and linking to the Maintenance page filtered to that house. Never one item per request: that space belongs to maintenance triage, and the reasoning that kept the setup checklist to a single row only holds while this stays aggregated. The stat tiles carry the counts as two numbers, open repairs and open jobs, from `regionSummary.ts`. Staff only, region-scoped through `filterByRegion`, and failing closed on an empty `allowedRegions` — `routeAccess.test.ts` asserts an RA with no regions gets no open work at all.

### Projects and bids

A project or a capital project carries what a repair never does: the signed contract, what it was expected to cost and what it came to, when it is meant to happen, and the bids it drew. `isProjectType` and `formatTargetPeriod` sit in `shared/schema.ts` beside the type vocabulary, as `isClosedMaintenanceStatus` does, so the update route, the edit dialog and the project card cannot disagree about which types carry them.

- **The five project fields live on the request row and are null on a repair by construction.** The PATCH refuses any of them with a value on a `request`-type row (400, "Only projects and capital projects carry…"), and a project turned back into a repair has them cleared in the same write. A resident cannot set them because a resident cannot PATCH at all; the resident create route strips them with the type. **The contract is a link** to where the signed agreement lives (Drive or Adobe), scheme-checked by `httpUrlFromClient` like the property links, and never an upload — the copy that matters is the one with the signature on it. **Costs are amounts only**; the money moves in QuickBooks and Ramp. **A target period is a year, optionally a quarter, never a date**: a quarter without a year is refused, checked over the stored row as well as the patch, because an edit sends only the field it changes. Changing the contract link records `maintenance_request.documents_changed` — that it changed on which request, **never the link itself**. Costs are estimates, not money moving, and are not audited.
- **Bids are their own table, and every bid route is staff under the maintenance permission, region-checked through the request, and a 400 on a repair.** Reading needs the view flag; recording, editing, removing and accepting need manage. The contractor is a contact record (checked to exist and to be in the caller's region, the same rule as linking one to a request) or a typed `vendorName` — one of the two, checked on the merged row so an edit cannot clear the last one. A resident never reaches a bid because they never reach its parent, and the route tests assert it directly on a project on the resident's own house, because "never reaches" is the kind of claim a refactor quietly breaks.
- **At most one accepted bid, enforced on the server.** `POST /api/maintenance-request-bids/:id/accept` is the only writer of `accepted` (the insert schema omits it), and `acceptMaintenanceRequestBid(requestId, bidId)` un-accepts every other bid on the request in one transaction. The screen's confirmation says "Accepting this bid un-accepts the others."
- **The quote goes up through `POST /api/maintenance-requests/:id/bid-documents`**, a narrow route whose permission is exactly the bid write's — staff, manage, region, project — run before the multipart parser, with the document limits and the same magic-byte check and upload layer as `/api/upload-doc`, so `document.uploaded` fires without the route adding an event. The bid create and edit accept `documentUrl` only in the `/uploads/<key>` shape and only when the `uploads` row says this caller stored it (`ownUploadFromClient`, shared with comment attachments); an edit that leaves the document out of the body leaves it alone, so an RA editing a colleague's bid is not refused for a file they did not upload. Reading it back is the `maintenanceRequestBid` reference in `canReadUploadReference`: the request's region for staff, and **a resident is refused by name** rather than left to the request rule, because that rule would let a household leader through once the project was turned back into a repair, and the bids on it do not go anywhere when that happens.
- **Removing a bid is a hard delete and the file stays** (known issue 1). The confirmation says "This removes the bid, not the file."

On the request page these are two cards, `RequestProjectCard` and `RequestBids` (the quote goes through `CommentAttachmentField` pointed at the bid-document route), rendered for staff on a project or a capital project and for nobody else; the edit dialog shows the five fields only when the chosen type is not Repair. The first plan's 5.5 ("no separate project entity") stands for retrospective contractor history; prospective projects are the addendum's Phase 10 and ADR-0001.
...
**The statement is an internal worksheet for finance, not a document the portal issues.** There is deliberately no send path: **"Open in mail" builds a `mailto:` link** (`statementMailto` in `shared/depositStatement.ts`) that opens the RA's own mail client with the recipient, subject and body prefilled, and the RA sends from their own account — the portal never sends a financial document to a resident. A click is not proof of a send, so it records `statementProvidedOn` as today and leaves it editable. A body over `MAILTO_MAX_LENGTH` (1,800 characters, the conservative cross-client ceiling) goes to the clipboard and the mail opens addressed with the subject only; a house of eight with five lines each fits, and the test says so. The text itself is `statementText`, pure, with each split line saying "your share of $X across N people" (`shareOfSplit`, display only — nothing recomputes from a group). `statement_sent` is progress, not completion: the money is still held and the dashboard keeps saying so until it goes back.

**The move-out damages worksheet** (`/walkthroughs/:id/damages`, `client/src/pages/DamagesWorksheet.tsx`) is how a move-out walkthrough becomes deductions (2026-09 RA review, item 7). Staff only; opens on a move-out that has been **submitted or reviewed** (a draft says "Mark it submitted first"). It lists every item recorded poor or damaged and not dismissed, each with an amount and who pays: **defaulting to the room's occupants** — residents whose `roomName` folds to the item's room — **and otherwise to the whole house on the walkthrough date** (`residentsActiveOn` in `shared/residents.ts`, the same rule the split dialog uses), with a per-person override and the `splitEvenly` shares shown live. A zero row stays listed as the record it was considered. **The screen writes nothing of its own:** each non-zero row goes through the single-deduction route (one person) or the split route (several), both now accepting `walkthroughItemId`, so the per-person rows, the cents arithmetic and the audit events are the ones that already exist; a row that already has deductions is shown as charged and not re-saved. Saves are per row, not all-or-nothing, and the toast says "Saved N of M" when one fails. Who can be charged is everybody living there on the walkthrough date **plus anybody already moved out whose deposit is still held** — a move-out walked after the move-outs were recorded is the normal case. The close-out list below marks a deposit **returned for the balance after deductions** (a shortfall keeps the box off; finance decides), takes the processor reference, and opens the statement. The dashboard's "Deposit to return" item now carries that same balance (`actionItems.ts` reads the deductions) — negative when overdrawn, in which case the row links to Finances instead of offering "Mark returned" — where it used to record the full amount held whatever had been deducted, a real bug fixed here. Recording `statementProvidedOn` on the mail click is by JR's decision in the 2026-09 review ("a click is not proof of a send", hence editable).
