...
| 1. Ownership + lease dates | **Not built.** `properties` has no `ownershipType` or lease columns. |
| 2. Resident accounts ↔ properties | **Not built.** No `propertyId` on `users`, no link table. |
| 5. Finance permission flags | **Not built — and deliberately so.** The rent/deposit routes gate on `requireStaff` (role-based), with a recorded decision (issue #43) that no finance permission is needed because staff *are* the finance audience. The meeting decided otherwise (flags now, so the later split is a grant). Adding the flags reverses #43's reasoning — do it knowingly, and backfill the flags `true` for existing staff so the change locks nobody out. |
| 7. Property-scoped `ownsRecord` | Current `ownsRecord` (`server/authz.ts:237`) is email-only, as the backlog says. Blocked by task 2. |
| 8. In-portal resident submission | **Half-built and broken-as-shipped**: `SubmitRequest.tsx` renders `MaintenanceRequestForm` but its `onSubmit` only `console.log`s and redirects — nothing reaches the API. Real work is wiring the mutation, not building the page. |
