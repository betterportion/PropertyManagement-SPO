...
On top of the role, each user has a row of fine-grained permissions — seventeen view/manage flags, including two that gate the finance screens on their own and one for the per-property setup checklist — and a list of allowed regions. Admins ignore both.
